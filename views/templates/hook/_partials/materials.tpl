  ...
  <h3>3 - {l s='Choisissez la matière' mod='ps_custom_product'}</h3>
   ...
   <div  id="pcp-materials-buttons" class="btn-group">
    {foreach from=$materials key=code item=material name=materials}
      <label for="pcp-material-input-{$code}" class="btn btn-outline-primary m-1  {if $smarty.foreach.materials.first}active{/if}">
         <input type="radio" id="pcp-material-input-{$code}" class="pcp-material-input" name="PCP_MATERIAL" value="{$code}" style="display:none" {if $smarty.foreach.materials.first}checked{/if} />
        {$material.label}
      </label>
...
<h3>4 - {l s='Choisissez la couleur' mod='ps_custom_product'}</h3>
  {foreach from=$materials key=code item=material name=materialsColor}
    <div class="pcp-material-fields border rounded px-2 mb-2" data-material="{$code}" style="{if $smarty.foreach.materialsColor.first}display:block;{else}display:none;{/if}">
