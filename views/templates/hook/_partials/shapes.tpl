<div id="pcp-shapes-list" class="group-input mt-2 mb-0">
  ...
  <div class="btn-group">
    {foreach from=$shapes key=code item=shape name=shapes}
      <label for="pcp-shape-input-{$code}" 
             class="btn btn-outline-primary m-1 {if $smarty.foreach.shapes.first}active{/if}">
        <input 
          type="radio"
          id="pcp-shape-input-{$code}"
          class="pcp-shape-input"
          name="PCP_SHAPE"
          value="{$code}"
          style="display:none"
          {if $smarty.foreach.shapes.first}checked{/if}
          onchange="pcpShowShape(this.value)"
        />
        {$shape.label}
      </label>
    {/foreach}
...
<div id="pcp-shapes-fields" class="mt-0">
  ...
  {foreach from=$shapes key=code item=shape name=shapesDimension}
    <div id="pcp-shape-{$code}-fields"
         class="pcp-shape-fields border rounded px-2 mb-2"
         data-shape="{$code}"
         style="{if $smarty.foreach.shapesDimension.first}display:block;{else}display:none;{/if}">
